    ...
    void setup() {
        ...
        pinMode(towerlights_pin1, OUTPUT);
    ...
    void loop() {
        ...
        unsigned long ldrTotal = 0;
        unsigned long gasTotal = 0;

        for (int i = 0; i < NUM_MEASUREMENTS ; i++) {

            analogRead(LDR_SENSOR); // discard first read for stabilization
        ...
        if (motionValue == LOW) {
            ...
            motionDetected = true;
            delay(10);
        } else if (motionDetected && millis() - lastMotionTime >= 10000) {
        ...
        if (tempC >= 30.0){
            String sourcee = "manual";
            digitalWrite(HEATER_PIN, LOW);
            digitalWrite(HEATER_FAN_PIN,LOW);
            Serial.println("heater_off|" + sourcee);
            delay(10);
        }
